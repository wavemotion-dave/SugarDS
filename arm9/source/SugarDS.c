...
u16 GAME_SPEED_PAL[]  __attribute__((section(".dtcm"))) = {653, 595, 546, 500, 728, 818 };
...
void ShowDebugZ80(void)
{
    ...
    if (myGlobalConfig.debugger == 3)
    {
        ...
        if (debug_area == 1)
        {
            ...
            DSPrint(0,idx++,7, tmp);
            sprintf(tmp, "CHRN %02X %02X %02X %02X", fdc.C, fdc.H, fdc.R, fdc.N);
            DSPrint(0,idx++,7, tmp);
    ...
    if (frame_complete)
    {
      ...
      else if (nds_key & (KEY_L | KEY_R))
      {
        if((nds_key & KEY_R) && !dampen)
        {
            if (nds_key & KEY_UP)
            {
                if (bWasOffsetShift) dampen = 1; else dampen = 10;
                bWasOffsetShift = 1;
                myConfig.offsetY++;
            }
