...
// which is true about 95% of the time. If the jump is not taken, we compensate TStates.
// ----------------------------------------------------------------------------------------
case JR_NZ:   if(CPU.AF.B.l&Z_FLAG) {CPU.TStates-=4; CPU.PC.W++;} else { M_JR; } break;
case JR_NC:   if(CPU.AF.B.l&C_FLAG) {CPU.TStates-=4; CPU.PC.W++;} else { M_JR; } break;
case JR_Z:    if(CPU.AF.B.l&Z_FLAG) { M_JR; } else {CPU.TStates-=4; CPU.PC.W++;} break;
case JR_C:    if(CPU.AF.B.l&C_FLAG) { M_JR; } else {CPU.TStates-=4; CPU.PC.W++;} break;
...
case CALL_PO: if(CPU.AF.B.l&P_FLAG) CPU.PC.W+=2; else { CPU.TStates+=8;M_CALL; } break;
case CALL_P:  if(CPU.AF.B.l&S_FLAG) CPU.PC.W+=2; else { CPU.TStates+=8;M_CALL; } break;
case CALL_Z:  if(CPU.AF.B.l&Z_FLAG) { CPU.TStates+=8;M_CALL; } else CPU.PC.W+=2; break;
case CALL_C:  if(CPU.AF.B.l&C_FLAG) { CPU.TStates+=8;M_CALL; } else CPU.PC.W+=2; break;
case CALL_PE: if(CPU.AF.B.l&P_FLAG) { CPU.TStates+=8;M_CALL; } else CPU.PC.W+=2; break;
case CALL_M:  if(CPU.AF.B.l&S_FLAG) { CPU.TStates+=8;M_CALL; } else CPU.PC.W+=2; break;
